declare namespace Two {
  ...
  class ImageSequence extends Two.Rectangle {
    constructor(
      paths: string | String[] | Two.Texture | Two.Texture[],
      ox?: number,
      oy?: number,
      frameRate?: number
    );
  ...
  class SVGRenderer extends Two.Events {
    constructor(parameters?: { domElement?: Element });
    /**
...
declare class Two {
   ...
   */
  makeSprite(
    pathOrTexture: string | Two.Texture,
    x: number,
    y: number,
    columns?: number,
    rows?: number,
   ...
   */
  makeImageSequence(
    pathsOrTextures: String[] | Two.Texture[],
    x: number,
    y: number,
    frameRate?: number,
    autostart?: boolean
   ...
   */
  load(
    pathOrSVGContent: string | SVGElement,
    callback: (...params: any[]) => any
  ): Two.Group;
  /**
